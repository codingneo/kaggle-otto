...
train = fread('./data/train.csv',header=TRUE,data.table=F)
test = fread('./data/test.csv',header=TRUE,data.table = F)
train = train[,-1]
test = test[,-1]
...
write.csv(pred,file='./submission/bagging/bst-original.csv'
...
for (z in tmpC) {
  ...
  ext_model_name = paste0('./submission/bagging/bst-ext', z, '.csv')
  write.csv(pred0,file=ext_model_name)
  pred = pred + t(pred0)
}
pred = pred/(z+1)
...
write.csv(pred,file='./submission/ottoHomeBagG4.csv', quote=FALSE,row.names=FALSE)